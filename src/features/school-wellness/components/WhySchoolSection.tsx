...
import { findings, whyBody, whyEyebrow, whyHeading } from "../data/content";

/**
 * `#why`: the argument for the page existing, in an accent-filled panel, beside
 * the five things the programme is said to turn up.
 *
 * The `1.3fr / 0.7fr` split collapses to one column at 900px, as the
 * reference's `[data-r="feat"]` rule does, and the hairline between the panels
 * is the grid's own 1px gap showing the parent background through.
 *
 * Every line in `findings` is unverified copy. See PLACEHOLDER_NOTICE in
 * `data/content.ts`.
 */
    ...
    >
      <Reveal className="grid grid-cols-[1.3fr_0.7fr] gap-px bg-[var(--home-hairline)] max-[899px]:grid-cols-1">
        <div className="bg-[var(--home-accent)] px-10.5 pt-11.5 pb-11 text-[var(--home-on-accent)]">
          ...
          <p className="mt-4.5 max-w-[56ch] text-[17px] leading-[1.62] opacity-86">{whyBody}</p>
        </div>
