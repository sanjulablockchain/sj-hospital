 ...
 * with its own explicit colour) onto the fill, it is tokenised so the light
 * theme fills with its own deeper accent, and it sits behind
 * `@media (hover: hover)` so a touch device does not latch a card into the
 * filled state.
 */
export function JumpCards() {
  return (
    <section id="jump" className="mx-auto max-w-[1440px] px-5 pt-20.5 sm:px-8 lg:px-11">
      <RevealStagger
        stepMs={80}
